 ...
 */
export default function EditUnitModalComponent(
  props: EditUnitModalComponentProps
) {
  ...
  const [rate, setRate] = useState(values.secInRate.toString());
  ...
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === CUSTOM_INPUT) {
      ...
      console.log("setCustomRate" + rate);
  ...
  const handleDeleteUnit = () => {
    ...
    for (const value of Object.values(meterDataByID)) {
      ...
      if (value.unitId == state.id) {
        ...
        error_message +=
          ` ${translate("meter")} "${value.name}" ${translate(
            "uses"
          )} ${translate("unit")} ` +
      ...
      if (conversionData[i].sourceId == state.id) {
        ...
        error_message +=
          ` ${translate("conversion")} ${
            unitDataByID[conversionData[i].sourceId].name
          }` +
          `${conversionArrow(conversionData[i].bidirectional)}` +
          `${unitDataByID[conversionData[i].destinationId].name} ${translate(
            "uses"
          )} ${translate("unit")}` +
    ...
    if (error_message) {
      ...
      deleteUnit(state.id)
        .unwrap()
        .then(() => {
          showSuccessNotification(translate("unit.delete.success"));
        })
        .catch((error) => {
          showErrorNotification(translate("unit.delete.failure") + error.data);
        });
    }
  };
  ...
  useEffect(() => {
    // // TO DO repurpose this function and then everything works
    // const isCustom = !["1", "60", "3600", "86400"].includes(
    //   state.secInRate.toString()
    // );
  ...
  const initialRate = values.secInRate.toString();
  const resetState = () => {
    setState(values);
    setRate(initialRate);
    setCustomRate(Number(CUSTOM_INPUT));
    setShowCustomInput(false);
  };
  ...
  const shouldUpdateUnit = (): boolean => {
    ...
    // Check for case 1
    if (
      props.unit.typeOfUnit === UnitType.meter &&
      ...
      if (meter) {
        ...
        showErrorNotification(
          `${translate("the.unit.of.meter")} ${meter.name} ${translate(
            "meter.unit.change.requires"
          )}`
        );
        inputOk = false;
      }
  ...
  const handleSaveChanges = () => {
    ...
    if (shouldUpdateUnit()) {
      ...
      const shouldRefreshReadingViews =
        props.unit.unitRepresent !== state.unitRepresent ||
        (props.unit.secInRate !== state.secInRate &&
          (props.unit.unitRepresent === UnitRepresentType.flow ||
            props.unit.unitRepresent === UnitRepresentType.raw));
  ...
  const inConversions = () => {
    for (const conversion of conversionData) {
      if (
        conversion.sourceId === state.id ||
        conversion.destinationId === state.id
      ) {
        return true;
      }
    }
        ...
        <ModalHeader>
          ...
          <div style={tooltipStyle}>
            <TooltipMarkerComponent
              page="units-edit"
              helpTextId={tooltipStyle.tooltipEditUnitView}
            />
          </div>
        </ModalHeader>
